...
for model_arg in MODELS.keys():
    ...
    for augmentation_flag in [True, False]: 
        ...
        trainGen = HDF5DatasetGenerator(config.TRAIN_HDF5, config.BATCH_SIZE, aug=Aug_dict[augmentation_flag], preprocessors=[sp, mp, iap], classes=config.NUM_CLASSES)
        valGen = HDF5DatasetGenerator(config.VAL_HDF5, config.BATCH_SIZE, aug=Aug_dict[augmentation_flag], preprocessors=[sp, mp, iap], classes=config.NUM_CLASSES)
        testGen = HDF5DatasetGenerator(config.TEST_HDF5, config.BATCH_SIZE, aug=Aug_dict[augmentation_flag], preprocessors=[sp, mp, iap], classes=config.NUM_CLASSES)
